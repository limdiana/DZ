...
на определенную сумму(сумма не может падать меньше 0, так что сделайте проверку).
Создайте метод удаляющий паспортные данные с аккаунта(также по поролю).
Класс "ПчелоСлон" (BeeElephant) Экземпляр класса инициализируется двумя целыми числами: первое относится к пчеле, второе - 
к слону. Класс реализует следующие методы: - fly() - может летать - возращает True, если часть слона не меньше части пчелы,
возращает строку: "tu-tu-doo-doo!", иначе "wzzzzz". eat(mael, value) - есть - может есть только нектар (nectar) млм траву 
(grass). Если съедает нектар, то из части слона вычитается количество съеденного, пчеде добавляется, иначе наоборот: у пчелы
вычитается, а слону добавляется. Не может увеличиться выше 100 и уменьшится меньше 0: get_parts() - возращает список
из значений: [часть пчелы, часть слона]
"""
